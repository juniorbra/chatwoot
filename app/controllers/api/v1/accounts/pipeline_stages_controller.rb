class Api::V1::Accounts::PipelineStagesController < Api::V1::Accounts::BaseController
  ...
  def reorder
    ...
    ActiveRecord::Base.transaction do
      # Temporarily set positions to a high value to avoid unique constraint violations
      stages_params.each do |stage_data|
        stage = current_account.pipeline_stages.find(stage_data[:id])
        stage.update_column(:position, stage.position + 100)
      end
  ...
  def pipeline_stage_params
    params.require(:pipeline_stage).permit(:name, :position, :color, :description)
  end
end
